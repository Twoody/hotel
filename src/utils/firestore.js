...
export async function addUserToFirestore (app, user) 
{
	...
	if (!user || !user.uid)
	{
		console.log("local firestore.js: Current function only supports `uid`; Aborting")
		return {
			invalid: true,
		}
	}
	...
	// One last check to make sure the upload was valid
	if (querySnapshot && querySnapshot.uid?.length)
	{
		querySnapshot.invalid = false
		// console.info("uid: ", querySnapshot.uid)
	}
	else
	{
