...
describe("MapFilters.vue", () => {
	it("renders two Filters components", () => {
		...
		const filters = wrapper.findAllComponents({ name: "Filters" });
	...
	it("properly computes active and inactive filters", () => {
		...
		wrapper.vm.filtersAll = {
			1: { id: 1, title: "Filter A", active: false },
			2: { id: 2, title: "Filter B", active: true },
			3: { id: 3, title: "Filter C", active: false },
		};

		expect(wrapper.vm.filtersActive).toEqual([{ id: 2, title: "Filter B", active: true }]);
		expect(wrapper.vm.filtersInactive).toEqual([
			{ id: 1, title: "Filter A", active: false },
			{ id: 3, title: "Filter C", active: false },
		]);
	});
	...
	it("emits 'updated-active' event when active filters change", async () => {
		...
		wrapper.vm.filtersAll = {
			1: { id: 1, title: "Filter A", active: false },
			2: { id: 2, title: "Filter B", active: false },
		};

		await wrapper.setData({
			filtersAll: {
				1: { id: 1, title: "Filter A", active: true },
				2: { id: 2, title: "Filter B", active: false },
			},
		});
		...
		expect(wrapper.emitted()["updated-active"][0]).toEqual([
			[{ id: 1, title: "Filter A", active: true }],
		]);
	});
	...
	it("handles errors gracefully when Firebase logEvent fails", () => {
		...
		const wrapper = createWrapper();
		wrapper.vm.filtersAll = { 1: { id: 1, title: "Filter A", active: false } };
	...
	it("sorts filters alphabetically", () => {
		const wrapper = createWrapper();
		const sorted = wrapper.vm.sortFilter({ title: "B" }, { title: "A" });
		expect(sorted).toBe(1);

		const sortedEqual = wrapper.vm.sortFilter({ title: "A" }, { title: "A" });
		expect(sortedEqual).toBe(0);

		const sortedReverse = wrapper.vm.sortFilter({ title: "A" }, { title: "B" });
