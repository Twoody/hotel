...
const createWrapper = (options = {}) => {
  ...
  const router = createRouter({
    ...
    routes: [{ path: "/booking/:id", name: "Booking" }],
  });
  ...
  return mount(ManageBooking, {
    global: {
      ...
      stubs: {
        ...
        BookingNotLoggedIn: { template: "<div>BookingNotLoggedIn</div>" },
        BookingNotFound: { template: "<div>BookingNotFound</div>" },
        CompletedBooking: { template: "<div>CompletedBooking</div>" },
        FinalizeBooking: { template: "<div>FinalizeBooking</div>" },
        UnauthorizedBooking: { template: "<div>UnauthorizedBooking</div>" },
      },
    },
    ...options,
  });
...
describe("ManageBooking.vue", () => {
  ...
  it("shows the BookingNotLoggedIn component if the user is not logged in", () => {
    ...
    expect(wrapper.findComponent({ name: "BookingNotLoggedIn" }).exists()).toBe(true);
  });
  ...
  it("shows the BookingNotFound component if no booking is found", async () => {
    ...
    expect(wrapper.findComponent({ name: "BookingNotFound" }).exists()).toBe(true);
  });
  ...
  it("shows the UnauthorizedBooking component if booking belongs to another user", async () => {
    ...
    expect(wrapper.findComponent({ name: "UnauthorizedBooking" }).exists()).toBe(true);
  });
  ...
  it("shows the CompletedBooking component if the booking is complete", async () => {
    ...
    getDoc.mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ guestID: "testUserId", paidAt: "2023-01-01" }),
    });
    ...
    expect(wrapper.findComponent({ name: "CompletedBooking" }).exists()).toBe(true);
  });
  ...
  it("shows the FinalizeBooking component if the booking is not complete", async () => {
    ...
    expect(wrapper.findComponent({ name: "FinalizeBooking" }).exists()).toBe(true);
  });
