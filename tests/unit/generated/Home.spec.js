...
const createWrapper = (options = {}) => {
  const router = createRouter({
    ...
    routes: [{ path: "/manageBooking", name: "manageBooking" }],
  });
  ...
  return mount(Home, {
    global: {
      ...
      stubs: {
        ...
        AccessibilityAccordion: { template: "<div>AccessibilityAccordion</div>" },
        CheckInAndOutAccordion: { template: "<div>CheckInAndOutAccordion</div>" },
        GuestSafetyAccordion: { template: "<div>GuestSafetyAccordion</div>" },
        ParkingAccordion: { template: "<div>ParkingAccordion</div>" },
        TrashAccordion: { template: "<div>TrashAccordion</div>" },
        WifiAccordion: { template: "<div>WifiAccordion</div>" },
      },
    },
    ...options,
  });
...
describe("Home.vue", () => {
  ...
  it("calls processBookingRequest on booking-request event", async () => {
    ...
    const wrapper = createWrapper();
    await wrapper.findComponent({ name: "AvailabilitySearch" }).vm.$emit("booking-request", { startDate: "2023-01-01", endDate: "2023-01-02" });
    expect(processBookingRequest).toHaveBeenCalledWith({ startDate: "2023-01-01", endDate: "2023-01-02" });
  });
  ...
  it("redirects to manageBooking after booking creation", async () => {
    ...
    await instance.processBookingRequest({ startDate: "2023-01-01", endDate: "2023-01-02" });
    expect(pushSpy).toHaveBeenCalledWith({
      name: "manageBooking",
      params: { id: undefined }, // `id` is undefined here due to mocked Firestore
    });
  });
